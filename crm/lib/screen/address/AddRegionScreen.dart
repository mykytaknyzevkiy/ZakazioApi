...
class AddRegionScreen extends StatelessWidget {
  final _viewModel = ViewModelProvider<RegionViewModel>().build(() => RegionViewModel());
  ...
  AddRegionScreen(this.regionModel) {
        ...
        child: Padding(
          ...
          child: SingleChildScrollView(
            child: Column(
              ...
              children: [
                Text(regionModel.id >= 0 ? "Изменить регион" : "Создать регион",
                    style:
  ...
  _onSaveClick() {
    ...
    if (regionModel.id >= 0) {
      _viewModel.edit(
          regionModel.id, _nameTxt.text(), int.parse(_zipCodeTxt.text()));
    } else {
      _viewModel.add(_nameTxt.text(), int.parse(_zipCodeTxt.text()));
    }
  }
