...
class ActiveMyPhoneScreen extends StatelessWidget {
  ...
  ActiveMyPhoneScreen(this.token);
        ...
        child: Padding(
          ...
          child: Column(
            ...
            children: [
              ...
              Text(
                  "На ваш номер телефона отправлен смс с кодом для подтверждения"),
              Divider(
                color: Colors.transparent,
                height: 30,
              ),
  ...
  _onSaveClick() async {
    final data = await _viewModel.verificatePhone(token, _smsCode.text());
    if (!data) {
      _smsCode.setError("Не верный код с смс");
    }
  }
